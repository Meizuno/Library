...
Rebase conflict on commit <sha>:
  CONFLICT (content): Merge conflict in library/<file>
...
Cleaned up:
  ✓ deleted local branch feat/notification-log-port (merged)
...
- ✅ Before invoking [`/add-feature`](../add-feature/SKILL.md) — ensures the new branch starts from latest `main`

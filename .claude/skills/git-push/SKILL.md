...
```
I'm about to push 3 commits to origin/feat/check-due-loans:

  abc1234 feat(loan): add CheckDueLoansUseCase + tests
  def5678 chore(deps): bump aiosmtplib to 3.1.0
  ghi9012 docs(readme): document the due-loans flow
...
```
✅ Pushed 3 commits to origin/feat/check-due-loans

Next options (none executed without your say-so):
  1. Open a PR        gh pr create --base main --head feat/check-due-loans
...
Naming the branch: see [branches.md](../git-commit/branches.md). Default pattern `<type>/<short-descriptive-name>` matching the commits' Conventional Commits type (e.g., `docs/git-push-skill`, `feat/check-due-loans`, `fix/borrow-race`).
...
- ❌ **Never push when `pytest -W error` is failing** — fix tests first
...
## feat/check-due-loans...origin/feat/check-due-loans [gone]
$ git log @{u}..HEAD --oneline
fatal: no upstream configured for branch 'feat/check-due-loans'
```
...
```
$ git status --short --branch
## feat/check-due-loans...origin/feat/check-due-loans [ahead 2]
```

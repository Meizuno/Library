...
main (protected, always green)
  ↑
  ├── feat/<short-name>     ← one feature, one slice — 30 min to 1 day
...
feat/notification-log-port
feat/loan-cancel-flow
fix/borrow-race-condition
...
```
feat/notification              ← too vague; one slice or all of it?
fix/bug                        ← what bug?
my-branch                      ← no type, no clue
yurii-2026-05-28               ← who/when, not what
feat/new-feature               ← tautological
feat/loans                     ← area, not change
feat/add-the-thing-that-does-stuff-for-loans-and-notifications  ← too long
```
...
**Use when:** the branch's commits are **individually meaningful and atomic** (each one passes `pytest -W error`, each one has a real Conventional-Commit message). This is **the goal** of disciplined branch work — and exactly what `/git-commit` produces.
